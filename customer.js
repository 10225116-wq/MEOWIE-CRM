document.addEventListener('DOMContentLoaded', () => {
    ...
    const feedbackTicketBody = document.getElementById('feedback-ticket-body');
    if (btnSubmitFeedback && feedbackTicketBody && chatContainer) {
        btnSubmitFeedback.addEventListener('click', async () => {
            const text = feedbackTicketBody.value.trim();
            if (!text) return;
            addMobileMessage(text, 'outgoing');
            feedbackTicketBody.value = '';
            ...
            try {
                ...
                if (response.ok) {
                    addMobileMessage(data.reply || 'Ticket submitted.', 'incoming');
                } else {
                    addMobileMessage('Error: ' + (data.error || 'Could not submit'), 'incoming');
                }
            } catch (err) {
                document.getElementById(loadingId).remove();
