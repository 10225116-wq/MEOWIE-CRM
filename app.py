...
def seed_feedback_if_empty():
    try:
        ...
        for i, row in enumerate(rows):
            ...
            docs.append(doc)
        if docs:
            feedback_collection.insert_many(docs)
            print(f'Seeded {len(docs)} feedback tickets.')
...
def list_feedback(current_user):
    ...
    try:
        if dept == 'Head of Manager':
            cur = feedback_collection.find({}).sort('created_at', -1)
        else:
            key = signup_department_to_filter_key(dept)
            if not key:
                return jsonify({'items': []}), 200
            cur = feedback_collection.find({'routing_departments': key}).sort('created_at', -1)
...
def submit_feedback(current_user):
    ...
    status = 'Not contact yet' if classified['category'] == 'Others' else 'In process'
...
def rule_based_chat_reply(current_user, user_message):
    ...
    if role == 'customer':
        ...
        if c['category'] != 'Others' or c.get('matched_word'):
            ...
            return (
                f'Thanks for reaching out. Based on your message, this looks related to '
                f'{c["issue"]} ({c["category"]}). Your case would go to: {teams}. '
                f'If you have not submitted a ticket yet, send your full details using Submit feedback from support.'
            )
        low = msg.lower()
